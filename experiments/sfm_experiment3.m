...
maxLoop = 1;
respMat = nan(numTrials*maxLoop+1, numTrials);
%----------------------------------------------------------------------
%                           The Experiment
%----------------------------------------------------------------------
...
for loopNum = 0:maxLoop-1
    respMat(loopNum*numTrials + 1,:)=loopNum+1;
    respMat(loopNum*numTrials + 3,:) = 0;
    respMat(loopNum*numTrials + 4,:) = 0;
    
    shuffler = Shuffle(1:numTrials);
    exp_order = exp_order(shuffler);
    respMat(loopNum*numTrials + 5,:) = 0;
    ...
    for trial = 1:numTrials
        ...
        %respMat(loopNum*numTrials + 5,:) = stimuli{i};
       ...
       try
        ...
        while tex > 0
            ...
            if keyIsDown && (keyCode(rightKey) || keyCode(leftKey))
                ...
                if keyCode(rightKey)
                    respMat(loopNum*numTrials + 2,i) = 1;
                else
                    respMat(loopNum*numTrials + 2,i) = 0;
                end 
                resp_main = false;
                break;
            end
    ...
    %User response
        ...
        while resp_main == true
            ...
            if keyIsDown
                ...
                elseif keyCode(replayKey)
                    respMat(loopNum*numTrials + 3,i)= 1 + respMat(loopNum*numTrials + 3,i);
                    try
                           movie = Screen('OpenMovie',window,path);
                    catch ME
                           fprintf('Error');
        ...
        respMat(loopNum*numTrials + 4,i) = elapsedTime;
        respMat(loopNum*numTrials + 6,i) = nan;
    end
end
%fill in video number for data processing
for i=1:numTrials
   respMat(loopNum*numTrials + 7,i) = i; 
end
...
% if ~isfolder(sprintf('Experiment_results_2022/%s',Name))
%     mkdir(sprintf('Experiment_results_2022/%s',Name));
% end
