...
X = [];
Y = [];
for experiment_file_path = experiment_files_paths
    currennt_X, current_Y = read_matrix(expeciment_file_path);
    X = [X; current_X];
    Y = [Y; current_Y];
end
...
function handle_experiments_data(X, Y)
% Linear (no-kernel)
linear_model = fitcsvm(X, Y, 'KernelFunction', 'linear', 'Standardize', true);
handle_experiments_data_model(X, Y, linear_model)

% Polynomial kernels
for order= [1, 2, 3]
    current_polynomial_model = fitcsvm(X, Y, 'KernelFunction', 'polynomial', 'Standardize', true, 'PolynomialOrder', order);
    handle_experiments_data_model(X, Y, current_polynomial_model)
end

% Gaussian kernels
for order= [1, 2, 3]
    current_gaussian_model = fitcsvm(X, Y, 'KernelFunction', 'gaussian', 'Standardize', true, '');
    handle_experiments_data_model(X, Y, current_gaussian_model)
end

% RBF kernels
for gamma = [0.01, 0.1, 1]
    current_rbf_model = fitcsvm(X, Y, 'KernelFunction', 'rbf', 'Standardize', true, 'KernelScale', 1 / sqrt(gamma));
    handle_experiments_data_model(X, Y, current_rbf_model)
end
end

function handle_experiments_data_model(X, Y, model)
    d = 0.02;
    ...
    hold on
    ezpolar(@(x)1);
    h(3) = plot(X(model.IsSupportVector,1), X(model.IsSupportVector,2), 'ko');
...
function blahblah (Name, Date, ExpIndex) 
% Clear the workspace
sca;
